...
void modbus_read_holding_registers_callbackfcn(modbus_obj_type *obj) {
  ...
  if (modbus_check_quality_isvalid(quality) == false) {
    obj->exception_code = 0x03;
    return;
  }

  if (modbus_check_ads_isvalid(start_ads, quality) ==
...
bool modbus_check_ads_isvalid(uint16_t start_ads, uint16_t quality) {
  if ((uint32_t)start_ads >= (uint32_t)REGISTER_LENGTH)
    return false; // start address not valid.
  if (((uint32_t)(start_ads) + quality) >= (uint32_t)REGISTER_LENGTH)
    return false; // quality not valid.
  return true;    // valid
}
